...
public class FileUtil {
	...
	/**
	 * @param path = diretorio base do projeto 
	 * @param result = armazenara o nome de todos os arquivos Java do diretorio
	 * @param base = eh usado na recursao. Indica o nome base do pacote
	 */
	public static void getClasses(String path, Vector<String> result, String base) {
		try {
			...
			for(int i=0;i<tam;i++) {				
				if(arquivos[i].isDirectory()) {
					...
					if (arquivos[i].getName().endsWith(".java") && !arquivos[i].getName().equals("SVGStorageFormat.java")) {
						//TODO Diego - TODEL
//						if(arquivos[i].getName().contains("LogFactory")) continue;
